#Reading the table:
product_1 <- read.csv("WTP_100ML_DATA.csv", header = T)
View(product_1)
...
maxPrice <- max(product_1[, 2:3])
#Price of standard product obtained from online demand learning.
baseprice <- 1.5       #Price of standard product

# Step 1: Calculating Surplus for Standard product----
#Surplus for Standard Product
product_1$Surplus_Standard <- product_1$Standard_WTP - baseprice
View(product_1)

# Step 2: Calculating Surplus for Premium Product for each consumer across all price levels----
...
for (p in seq(1,maxPrice,by= 0.1)) {
  ...
  for (i in 1:N) {
    Surplus_Premium[i, index] <- product_1[i, 3] - p
  }
}
View(Surplus_Premium)

# Step 3: Calculating demands for both standard and premium products----
...
for (p in seq(1,maxPrice,by= 0.1)) {
  ...
  demand_Standard[index] <- sum((product_1$Surplus_Standard > Surplus_Premium[, index]) 
                                & 
                                (product_1$Surplus_Standard >= 0)
                               )
  
  demand_Premium[index] <- sum((Surplus_Premium[, index] >= product_1$Surplus_Standard) 
                               & 
...
print(paste("When the non-premium price is £1.5, the optimal premium price is: £", priceBest))
